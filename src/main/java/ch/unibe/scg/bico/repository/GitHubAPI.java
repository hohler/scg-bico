...
public class GitHubAPI {
	...
	/*public void loadPullRequests() {
		...
		try {
			List<PullRequest> result = service.getPullRequests(repository, "closed");
			for(PullRequest p : result) {
				System.out.println("PullRequest:"+p.getNumber());
				pullRequests.put(new Integer(p.getNumber()), p);
			}
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	/*public PullRequest loadSinglePullRequest(int id) {
		PullRequestService service = new PullRequestService(client);
		
		try {
			PullRequest result = service.getPullRequest(repository, id);
			System.out.println("PullRequest:"+result.getNumber());
			pullRequests.put(new Integer(result.getNumber()), result);
			return result;
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	/*public PullRequest getPullRequest(int id) {
		if(pullRequests == null) {
			System.err.println("GitHub pull requests are not loaded!");
			return null;
		}
		return pullRequests.get(new Long(id));
	}*/
	
	/*public Issue getIssue(int pullRequestNumber) {
		return getIssue(getPullRequest(pullRequestNumber));
	}
	
	public Issue getIssue(PullRequest p) {
		IssueService service = new IssueService(client);
		int issueNumber = getIssueNumberFromPullRequest(p);
		try {
